...
@pytest.mark.asyncio
async def test_execute_forward_command(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_execute_backward_command(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_execute_left_rotation(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_execute_right_rotation(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_execute_complex_command_string(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_execute_case_insensitive_commands(async_client: AsyncClient, mock_env):
...
@pytest.mark.asyncio
async def test_robot_state_persists_between_calls(async_client: AsyncClient, mock_env):
    ...
    assert data2["initial_position"]["x"] == 3  # Should start from previous final position
    assert data2["final_position"]["x"] == 2

@pytest.mark.asyncio
async def test_position_endpoint_reflects_command_execution(async_client: AsyncClient, mock_env):
